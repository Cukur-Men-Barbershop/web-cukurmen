    ...
    <div class="content-card">
        ...
        <div class="form-section" style="margin-bottom: 2rem; padding: 1.5rem; background: var(--surface-light); border-top: var(--border-subtle); margin-top: 2rem; padding-top: 2rem;">
            ...
            <form id="addBarberForm">
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label>Rating</label>
                        <input type="number" id="barberRating" name="rating" min="0" max="5" step="0.1">
                    </div>
                    <div class="form-group">
                        <label>Status</label>
                ...
                <div class="form-group">
                    ...
                    <input type="file" id="barberImage" name="image" accept="image/*" placeholder="Pilih gambar barber">
                </div>
                <button type="submit" class="btn-primary" style="margin-top: 1rem;">Tambah Barber</button>
            </form>
        </div>

        <h4 style="font-size: 1.5rem; border-top: var(--border-subtle); padding-top: 2rem; margin-top: 2rem; color: var(--accent-gold);">Daftar Barber</h4>
        <div id="barberList">
        ...
        <div class="form-section" style="margin-bottom: 2rem; padding: 1.5rem; background: var(--surface-light); border-top: var(--border-subtle); margin-top: 2rem; padding-top: 2rem;">
            ...
            <form id="addProductForm">
                ...
                <div class="form-group">
                    <label>Upload Gambar</label>
                    <input type="file" id="productImage" name="image" accept="image/*" placeholder="Pilih gambar produk">
                </div>
            ...
            @if(isset($products) && $products->count() > 0)
                @foreach($products as $product)
                    <div class="product-card" data-id="{{ $product->id }}">
                        <img class="product-card-img" src="{{ $product->image_path ?? $product->image ?? $product->img ?? '/assets/img/default-product.jpg' }}" alt="{{ $product->name ?? 'Product' }}">
...
<div id="editProductModal" class="modal" style="display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.4);">
    <div class="modal-content" style="background-color: var(--surface-card); margin: 5% auto; padding: 20px; border: none; border-radius: 8px; width: 60%; max-width: 600px;">
        ...
        <form id="editProductForm">
            ...
            <div class="form-group">
                <label>Upload Gambar</label>
                <input type="file" id="editProductImage" name="image" accept="image/*" placeholder="Pilih gambar produk">
...
function loadBarberList() {
    ...
    .then(barbers => {
        ...
        if (barbers.length === 0) {
            barberListContainer.innerHTML = '<p style="color: var(--text-grey); text-align: center; padding: 2rem;">Tidak ada barber</p>';
            return;
        }

        let barberHTML = '';
        barbers.forEach(barber => {
            const barberImage = barber.image_path || '/assets/img/barber1.jpg'; // Use default if no image
            barberHTML += `
